...
if uploaded_image is not None:
    ...
    if st.button("Classify Image"):
        ...
        if result == "Genuine":
            st.markdown(
                f'<div style="background-color:#d4edda;padding:10px;border-radius:5px;color:#155724;text-align:center;">\
                <b>The image is classified as: {result}</b></div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                f'<div style="background-color:#f8d7da;padding:10px;border-radius:5px;color:#721c24;text-align:center;">\
                <b>The image is classified as: {result}</b></div>',
                unsafe_allow_html=True
            )
